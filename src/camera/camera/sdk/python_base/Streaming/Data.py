...
class Data:
    ...
    def read(self, dataBuffer, convertToMM=True):
        ...
        xml_changed = self.changedCounter < changedCounter[0]
        binarySegment = memoryview(dataBuffer)[offset[1]:offset[2]]

        if (numSegments == 3):
            # numBytes(magicword) = 4, numBytes(pkglength) = 4
            if logging.getLogger().isEnabledFor(logging.DEBUG):
                logging.debug("The whole overlay XML segment:")
                logging.debug(memoryview(dataBuffer)[offset[2]:pkglength+4+4])
        ...
        if myXMLParser.hasDepthMap:
            ...
            if xml_changed or not hasattr(self, "cameraParams"):
                self.cameraParams = CameraParameters(
                    width=myXMLParser.imageWidth,
